...
const vueSince = computed(() => {
  const item = data.value.experience.frameworks.find((i) => i.title.toLowerCase().includes("vue"));
  return item?.["date-from"] ? new Date(item["date-from"]).getFullYear() : null;
});
...
<template>
  <div>
    <!-- ── Loading skeleton ──────────────────────────────────────────── -->
    <div v-if="loading" class="space-y-9" aria-busy="true" aria-label="Loading portfolio">

      <!-- Hero strip skeleton -->
      <div class="flex flex-wrap items-center gap-3 rounded-[var(--radius-md)] border border-[var(--color-outline)] bg-[var(--color-surface)] px-5 py-4">
        ...
        <div class="h-4 w-px bg-[var(--color-outline)]" />
        <div class="skeleton-line" style="width:100px;animation-delay:120ms" />
        <div class="h-4 w-px bg-[var(--color-outline)]" />
        <div class="skeleton-rect h-6 w-20 rounded-full" style="animation-delay:180ms" />
        <div class="skeleton-rect h-6 w-24 rounded-full" style="animation-delay:240ms" />
      </div>

      <!-- Section blocks × 3 (platforms, protocols, frameworks) -->
      <div v-for="(cols, si) in [[3, 100], [1, 100], [5, 58]]" :key="si" class="space-y-3">
        ...
        <div class="grid gap-3" :style="`grid-template-columns: repeat(auto-fill, minmax(${si === 2 ? 180 : 260}px, 1fr))`">
          <div
            v-for="j in cols[0]"
            :key="j"
            class="skeleton-rect"
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          <div v-for="w in [82, 100, 74, 92]" :key="w" class="skeleton-rect h-8 rounded-full" :style="`width:${w}px`" />
        </div>
      </div>
      ...
      <div class="space-y-3" style="max-width:720px">
        ...
        <div class="rounded-[var(--radius-md)] bg-[var(--color-surface)] p-6 shadow-sm space-y-3">
          ...
          <div class="flex flex-wrap gap-2 pt-1">
            <div v-for="w in [64, 52, 72]" :key="w" class="skeleton-rect h-6 rounded-full" :style="`width:${w}px`" />
          </div>
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div>
        <p class="text-sm font-semibold text-[var(--color-error)]">Failed to load portfolio</p>
    ...
    <div v-else class="space-y-12">
      ...
      >
        <span v-if="pythonSince" class="hero-stat">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>
          {{ new Date().getFullYear() - pythonSince }}+ yrs Python
        </span>
        <div v-if="pythonSince && vueSince" class="h-4 w-px bg-[var(--color-outline)]" aria-hidden="true" />
        <span v-if="vueSince" class="hero-stat">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><polygon points="12 2 2 7 12 22 22 7 12 2"/></svg>
        ...
        <span class="hero-domain-tag">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
          FinTech
        </span>
        <span class="hero-domain-tag">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
          EV Charging
        </span>
      </Motion>
      ...
      >
        <Motion
          ...
          :initial="cardInitial"
          :animate="cardVisible"
          :transition="{ duration: 0.25, delay: delay(0, 80), easing: [0.2,0,0,1] }"
        >
          <h2 class="text-xl">Platforms</h2>
          <span class="count-badge">{{ data.experience.platforms.length }}</span>
        </Motion>
        ...
        <ul class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3" role="list">
          <Motion
            ...
            :initial="cardInitial"
            :animate="cardVisible"
            :transition="{ duration: 0.25, delay: delay(i, 120), easing: [0.2,0,0,1] }"
          >
            <div class="mb-2.5 flex items-start justify-between gap-2">
              <div class="exp-logo">
                <img v-if="item['img-src']" :src="`/${item['img-src']}`" :alt="item.title" />
      ...
      >
        ...
        >
          ...
          <span class="count-badge">{{ data.experience.protocols.length }}</span>
        </Motion>
        <p class="mb-4 text-[13px] text-[var(--color-on-surface-variant)]">
      ...
      >
        ...
        >
          ...
          <span class="count-badge">{{ data.experience.frameworks.length }}</span>
        </Motion>
        <p class="mb-4 text-[13px] text-[var(--color-on-surface-variant)]">
          ...
          >
            <div class="flex items-center justify-between gap-2">
              <div class="flex items-center gap-2.5">
                ...
                <div>
                  <p class="text-[13px] font-semibold text-[var(--color-on-surface)]">{{ item.title }}</p>
                  <p class="text-[11px] text-[var(--color-on-surface-variant)]">{{ durationLabel(item) }}</p>
                </div>
              </div>
              <span v-if="isActive(item)" class="active-dot flex-shrink-0" aria-label="Currently using" />
            </div>
          </Motion>
        </ul>
      </section>
      ...
      >
        ...
        <p class="mt-3 flex items-center gap-1.5 text-[12px] text-[var(--color-on-surface-variant)]">
      ...
      <section v-if="data.project" aria-label="Personal projects">
        ...
        <Motion
          ...
          :initial="cardInitial"
          :animate="cardVisible"
          :transition="{ duration: 0.3, delay: delay(1, 80), easing: [0.2,0,0,1] }"
        >
          <p class="project-eyebrow">syamim.hakimi.dev</p>
          <h3 class="mt-1 text-[20px] font-normal text-[var(--color-on-surface)]">
            {{ data.project.title }}
          </h3>
          <p v-if="data.project.subtitle" class="mt-1 text-[13px] text-[var(--color-on-surface-variant)]">
            {{ data.project.subtitle }}
          </p>
          <p v-if="data.project.description" class="mt-3 text-[14px] leading-relaxed text-[var(--color-on-surface-variant)]">
            {{ data.project.description }}
          </p>
          <div v-if="data.techstack.length > 0" class="mt-4">
            <p class="mb-2 text-[11px] font-semibold uppercase tracking-[.06em] text-[var(--color-on-surface-variant)]">
              Tech Stack
            </p>
...
.card-outlined:hover::before { transform: scaleY(1); }
.card-outlined:hover {
...
[data-theme="dark"] .active-label { color: #4ade80; }
.year-badge {
...
.chip {
  ...
  background: var(--color-surface-variant);
  border-radius: 999px;
  font-size: 13px; font-weight: 500; color: var(--color-on-surface);
  border: 1px solid var(--color-outline);
}
