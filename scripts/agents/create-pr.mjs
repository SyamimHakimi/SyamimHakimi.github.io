...
const res = await fetch('https://api.github.com/repos/SyamimHakimi/SyamimHakimi.github.io/pulls', {
  method: 'POST', headers: H,
  body: JSON.stringify({ title, head, base: 'main', body }),
});
const pr = await res.json();
if (pr.html_url) { console.log(`PR created: ${pr.html_url} — #${pr.number}`); }
else { console.error('Failed:', JSON.stringify(pr)); process.exit(1); }
