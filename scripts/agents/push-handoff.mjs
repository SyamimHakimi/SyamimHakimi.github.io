...
if (!message) {
  console.error(
    'Usage: node --env-file=.env scripts/_push-handoff.mjs "<message>" ["<from>→<to>" ...]',
  );
  process.exit(1);
}

// Always start from the live remote content to avoid overwriting concurrent edits
const getRes = await fetch(`${REPO}/contents/HANDOFF.md?ref=main`, {
  headers: H,
});
...
for (const pair of pairs) {
  ...
  if (sep === -1) {
    console.warn(`Skipping malformed pair (no → separator): ${pair}`);
    continue;
  }
...
if (result.content) {
  console.log("HANDOFF.md updated on main:", result.content.sha);
} else {
