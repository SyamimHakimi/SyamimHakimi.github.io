...
if (!TELEGRAM_TOKEN || !TELEGRAM_CHAT_ID) {
  console.error(
    "Error: TELEGRAM_TOKEN and TELEGRAM_CHAT_ID must be set in .env",
  );
...
async function screenshot(htmlPath, viewport) {
  ...
  const outPath = join(
    tmpdir(),
    `ui-preview-${viewport.name}-${Date.now()}.png`,
  );
...
async function main() {
  ...
  if (!file) {
    console.error(
      'Usage: node scripts/ui-preview.mjs --file <path> [--label "Title"]',
    );
    process.exit(1);
  }
  ...
  console.log(
    `📸 Sending "${label}" (${viewports.map((v) => v.name).join(" + ")}) to Telegram…`,
  );
  await sendMessage(
