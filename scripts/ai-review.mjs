...
const { GITHUB_TOKEN, PR_NUMBER, PR_TITLE, HEAD_BRANCH, REPO } = process.env;
...
async function mergeWhenReady(maxAttempts = 10, delayMs = 30_000) {
  for (let i = 0; i < maxAttempts; i++) {
    const res = await fetch(
      `${GH_API}/repos/${REPO}/pulls/${PR_NUMBER}/merge`,
      {
        method: "PUT",
        headers: GH_HEADERS,
        body: JSON.stringify({
    ...
    if (res.status === 405 || data.message?.includes("not mergeable")) {
      console.log(
        `Attempt ${i + 1}/${maxAttempts} — CI pending, retrying in ${delayMs / 1000}s…`,
      );
      await sleep(delayMs);
      continue;
    }
...
async function deleteBranch() {
  ...
  if (res.status !== 204) {
    console.warn(
      `Branch delete returned ${res.status} — may already be deleted`,
    );
  }
}
...
async function updateHandoffViaApi(phaseNum) {
  ...
  if (!getRes.ok)
    throw new Error(
      `GET HANDOFF.md failed ${getRes.status}: ${await getRes.text()}`,
    );
  ...
  const putRes = await fetch(`${GH_API}/repos/${REPO}/contents/HANDOFF.md`, {
    method: "PUT",
    headers: GH_HEADERS,
    body: JSON.stringify({
  ...
  if (!putRes.ok)
    throw new Error(
      `PUT HANDOFF.md failed ${putRes.status}: ${await putRes.text()}`,
    );
...
async function main() {
  ...
  if (merged) {
    ...
    await deleteBranch();
    console.log(
      `Phase A${phaseNum} complete — branch deleted, HANDOFF.md updated to MERGED`,
    );
  } else {
    await postComment(
